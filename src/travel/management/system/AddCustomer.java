...
public class AddCustomer extends JFrame {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            // Data validations
            if (s9.isEmpty() || s1.isEmpty() || s2.isEmpty() || s3.isEmpty() || s4.isEmpty() || s5.isEmpty() || s7.isEmpty() || s8.isEmpty() || s10.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Please fill in all fields.");
            } else if (!isValidPhoneNumber(s8)) {
                JOptionPane.showMessageDialog(null, "Invalid phone number format.");
            } else if (!isValidPassport(s1)) {
                JOptionPane.showMessageDialog(null, "Invalid passport format.");
            } else if (!isValidEmail(s10)) {
                JOptionPane.showMessageDialog(null, "Invalid email format.");
            } else if (!isValidName(s3)) {
                ...
                PreparedStatement pst = c.conn.prepareStatement(q1);
                pst.setString(1, s9);
                pst.setString(2, s1);
                pst.setString(3, s2);
                pst.setString(4, s3);
    ...
    // Basic phone number validation (allows digits and optional plus sign)
    return phoneNumber.matches("^(\\+)?\\d+$");
}

private boolean isValidPassport(String passport) {
    // Passport validation (customize this regex as needed)
    return passport.matches("^[A-Z0-9]{8}$");
}

private boolean isValidEmail(String email) {
    // Email validation (a simplified regex, a more robust one is recommended)
